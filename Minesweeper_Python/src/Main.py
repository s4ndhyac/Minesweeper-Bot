...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="", prog="Main.py", usage="%(prog)s [options]", epilog="Note: [options] can be in any order")

    # Parse options																				# Help is default
    parser.add_argument(
        "-f", "-F", help="file or directory name", nargs='*')								# File path
    parser.add_argument("-m", "-M", help="enable ManualAI mode",
                        action="store_true")			# ManualAI
    parser.add_argument("-r", "-R", help="enable RandomAI mode",
                        action="store_true")			# RandomAI
    parser.add_argument("-v", "-V", help="enable verbose mode",
                        action="store_true")			# Verbose
    parser.add_argument("-d", "-D", help="enable debug mode",
    ...
    if inputFile:
        ...
        if (os.path.isdir(inputFile)):
            ...
            for dirpath, _, filenames in directory:
                for filename in filenames:
                    ...
                    world = World(filename=f, aiType=aiType,
        ...
        # If inputFile is a world file
            ...
            world = World(filename=inputFile, aiType=aiType,
                          verbose=verbose, debug=debug)
            score = world.run()
            if score > 0:

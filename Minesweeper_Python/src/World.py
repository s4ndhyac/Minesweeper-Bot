...
class World():
    ...
    def __init__(self, filename=None, aiType="myai", verbose=False, debug=False):
        ...
        try:
            ...
            if filename != None:
                with open(filename, 'r') as file:
                    ...
                    self.__lastTile = (
                        firstMoveCoords[0]+1, firstMoveCoords[1]+1)
        ...
        if aiType == "manual":
            ...
            self.__ai = RandomAI(self.__rowDimension, self.__colDimension,
                                 self.__totalMines, firstMoveCoords[0], firstMoveCoords[1])
        elif aiType == "myai":
            self.__ai = MyAI(self.__rowDimension, self.__colDimension,
                             self.__totalMines, firstMoveCoords[0], firstMoveCoords[1])
    ...
    def __createBoard(self, inputStream: "filePointer" = None) -> None:
        ...
        if inputStream:
            self.__rowDimension, self.__colDimension = [
                int(x) for x in inputStream.readline().split()]
            self.__board = [[self.__Tile() for i in range(self.__rowDimension)]
    ...
    def __printAgentInfo(self) -> None:
        ...
        print("Tiles covered: " + str(self.__coveredTiles) + " | Flags left: " +
              str(self.__flagsLeft) + " | Last action: {} on {}".format(self.__lastAction, self.__lastTile))
